...
import {
  ...
  Sparkles, ChevronLeft, UserCheck, UserX
} from 'lucide-react';
...
// ── Participant Detail Modal Types ──
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto" onClick={onClose}>
      <div className="relative bg-white rounded-3xl p-6 max-w-2xl w-full shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-6">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold text-lg shadow-md">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
            ...
            <p className={`text-sm font-bold ${progress?.completed ? 'text-emerald-600' : progress ? 'text-amber-600' : 'text-slate-400'}`}>
              {p.statusText}
            </p>
          </div>
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 h-2 bg-slate-200 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${(progress?.progress_percent || 0) >= 80 ? 'bg-emerald-500' : (progress?.progress_percent || 0) >= 40 ? 'bg-amber-500' : 'bg-red-500'}`}
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
            ...
            <p className="text-sm font-bold text-slate-800">{p.scoreText}</p>
          </div>
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
            <p className="text-xs text-slate-500 mb-1">الوقت المستغرق</p>
            <p className="text-sm font-bold text-slate-800">{p.timeSpentFormatted}</p>
          </div>
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
            <p className="text-xs text-slate-500 mb-1">تاريخ البدء</p>
            <p className="text-sm font-bold text-slate-800">
              {progress?.started_at ? new Date(progress.started_at).toLocaleDateString('ar-IQ') : '—'}
            </p>
          </div>
        </div>
        ...
        {p.aiAnalysis ? (
          <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-2xl p-5 border border-purple-200">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-bold text-purple-800 text-sm">تحليل AI للأداء</h4>
            </div>
          ...
          <div className="bg-gradient-to-br from-slate-50 to-indigo-50 rounded-2xl p-5 border border-slate-200 text-center">
            ...
            <Button variant="primary" size="sm" onClick={onGenerateAI} icon={<Sparkles size={14} />} iconPosition="left"
          ...
          <div className="mt-6 bg-white rounded-2xl border border-slate-100 p-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-700">بدء الدورة</p>
                  <p className="text-[11px] text-slate-500">{new Date(progress.started_at).toLocaleString('ar-IQ')}</p>
                </div>
              </div>
              {progress.last_access_at && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-slate-700">آخر وصول</p>
                    <p className="text-[11px] text-slate-500">{new Date(progress.last_access_at).toLocaleString('ar-IQ')}</p>
                  </div>
                </div>
              )}
              {progress.completed_at && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-slate-700">إتمام الدورة</p>
                    <p className="text-[11px] text-slate-500">{new Date(progress.completed_at).toLocaleString('ar-IQ')}</p>
                  </div>
                </div>
              )}
            </div>
...
export default function TrainingReportsPage() {
  ...
  const [timeRange, setTimeRange] = useState<'6months' | 'year' | 'all'>('all');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(err.message || 'فشل في تحميل بيانات التدريب');
      } finally {
        setLoading(false);
      }
    };
  ...
  const courseStats: CourseStats[] = useMemo(() => {
    return courses.map(course => {
      ...
      const avgScore = completed > 0
        ? Math.round(progressForCourse.filter(p => p.completed).reduce((sum, p) => sum + (p.score || 0), 0) / completed)
        : 0;
      return {
        id: course.id,
        title: course.title,
  ...
  const monthlyTrends: MonthlyTrend[] = useMemo(() => {
    ...
    courseProgress.forEach(p => {
      ...
      if (!monthMap.has(monthKey)) {
        monthMap.set(monthKey, { enrollments: 0, completions: 0, avgScores: [] });
      }
      const entry = monthMap.get(monthKey)!;
      entry.enrollments += 1;
      if (p.completed && p.completed_at) {
        entry.completions += 1;
        if (p.score) entry.avgScores.push(p.score);
      }
    });
    return Array.from(monthMap.entries())
      .map(([key, data]) => ({
        ...
        avgScore: data.avgScores.length > 0
          ? Math.round(data.avgScores.reduce((a, b) => a + b, 0) / data.avgScores.length)
          : 0,
      }))
  ...
  const participants = useMemo(() => {
      ...
      progressForCourse.forEach(p => {
        ...
        if (profile) {
          ...
          list.push({
            ...
            timeSpentFormatted: hrs > 0 ? `${hrs}s ${mins}m` : mins > 0 ? `${mins}m` : `${timeSpent}%`,
    ...
    // Add employees with no progress
  ...
  const filteredParticipants = useMemo(() => {
    return participants.filter(p => {
      const matchSearch = !participantSearch || 
        p.employee.full_name.includes(participantSearch) || 
        p.employee.email.includes(participantSearch) ||
        p.employee.department?.includes(participantSearch);
  ...
  const overallAvgScore = courseStats.length > 0
    ? Math.round(courseStats.reduce((a, c) => a + c.avgScore, 0) / courseStats.length)
    : 0;
  ...
  const deptChartData = useMemo(() => {
    return departmentStats.map(d => ({
      name: d.dept,
      معدل_الإتمام: d.completionRate,
      المتبقي: 100 - d.completionRate,
    }));
  }, [departmentStats]);
      ...
      if (!progress) {
        ...
        if (progress.completed && progress.score) {
          if (progress.score >= 85) {
            recommendations.push(`أداء متميز: حقق ${progress.score}% في الدورة`);
            recommendations.push('يمكن ترشيحه كمدرب مساعد للدورة');
          } else if (progress.score >= 70) {
            recommendations.push(`أداء جيد: حقق ${progress.score}%`);
            recommendations.push('ينصح بمراجعة المواضيع التي أخطأ فيها');
          } else {
            recommendations.push(`يحتاج تحسين: حقق ${progress.score}% فقط`);
            recommendations.push('يوصى بإعادة الدورة والتركيز على النقاط الرئيسية');
          }
        }
        if (progress.time_spent) {
          const avgSecondsPerModule = Math.round(progress.time_spent / (courses.find(c => c.id === progress.course_id)?.points || 1));
          if (avgSecondsPerModule < 60) {
            recommendations.push('⚠️ سرعة غير طبيعية في إنهاء المحتوى');
          } else if (avgSecondsPerModule > 600) {
            recommendations.push('وقت دراسة طويل قد يشير إلى صعوبة في الفهم');
          }
        }
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]" dir="rtl">
        <div className="text-center">
          ...
          <p className="text-slate-500 font-semibold">جاري تحميل بيانات التقارير...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">إحصائيات شاملة وتحليلات الأداء للبرامج التدريبية — بيانات حقيقية من النظام</p>
        </div>
        <div className="flex items-center gap-2">
          <select value={timeRange} onChange={e => setTimeRange(e.target.value as any)}
            className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-semibold text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-300">
            <option value="6months">آخر 6 أشهر</option>
            <option value="year">آخر سنة</option>
            <option value="all">الكل</option>
          </select>
          <Button variant="primary" size="sm" onClick={handleExport} icon={<Download size={16} />} iconPosition="left">
            تصدير التقرير
          </Button>
        </div>
      </div>
        ...
        {[
          ...
          return (
            <div key={i} className="bg-white rounded-2xl border border-slate-100 p-3 shadow-sm hover:shadow-md transition-shadow">
              <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${kpi.color} flex items-center justify-center mb-2`}>
                <KIcon size={16} className="text-white" />
              </div>
              <p className="text-lg font-extrabold text-slate-800">{kpi.val}</p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-sm">
              ...
              الاتجاهات الشهرية — التسجيل والإتمام
            </CardTitle>
          </CardHeader>
          <div className="h-64">
            {monthlyTrends.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-slate-400 text-sm">لا توجد بيانات شهرية متاحة</div>
            )}
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="h-64 flex items-center justify-center">
            {categoryData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <RePie>
                  <Pie data={categoryData} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3}
            ...
            ) : (<div className="text-slate-400 text-sm">لا توجد بيانات تصنيف متاحة</div>)}
          </div>
        </Card>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <Card>
          ...
          <div className="h-64">
            {courseStats.filter(c => c.enrolled > 0).length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={courseStats.filter(c => c.enrolled > 0).map(c => ({
            ...
            ) : (<div className="flex items-center justify-center h-full text-slate-400 text-sm">لا توجد درجات متاحة بعد</div>)}
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="h-64">
            ...
            ) : (<div className="flex items-center justify-center h-full text-slate-400 text-sm">لا توجد بيانات أقسام متاحة</div>)}
          </div>
        </Card>
      </div>
            ...
            <div key={idx} className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl p-4 border border-slate-200">
              <div className="flex items-center justify-between mb-3">
                <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
                  lvl.name === 'مبتدئ' ? 'bg-emerald-100 text-emerald-700' :
                  lvl.name === 'متوسط' ? 'bg-sky-100 text-sky-700' :
                  lvl.name === 'متقدم' ? 'bg-violet-100 text-violet-700' : 'bg-rose-100 text-rose-700'
                }`}>{lvl.name}</span>
                <span className="text-lg font-extrabold text-slate-800">{lvl.rate}%</span>
              </div>
              <div className="h-2 bg-slate-200 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${lvl.rate >= 80 ? 'bg-emerald-500' : lvl.rate >= 60 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${lvl.rate}%` }} />
              </div>
              <div className="flex justify-between mt-2 text-[11px] text-slate-500">
                <span>مسجل: {lvl.enrolled}</span>
                <span>مكتمل: {lvl.completed}</span>
              </div>
            </div>
          )) : (
            <div className="col-span-4 text-center text-slate-400 text-sm py-8">لا توجد بيانات مستويات متاحة</div>
          )}
        </div>
      </Card>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {courseStats.filter(c => c.enrolled > 0).length > 0 ? (
                  ...
                  <tr key={course.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="px-4 py-3">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-400">{course.category} · {course.level}</p>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${course.completionRate >= 80 ? 'bg-emerald-500' : course.completionRate >= 60 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${course.completionRate}%` }} />
                        </div>
                    ...
                    <td className="px-4 py-3 text-center">
                      {course.avgScore >= 85 ? (
                        <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full"><ArrowUp size={10} /> ممتاز</span>
                      ) : course.avgScore >= 75 ? (
                        <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full"><Minus size={10} /> جيد</span>
                      ) : (
                ...
                <tr><td colSpan={7} className="text-center py-8 text-slate-400 text-sm">لا توجد دورات مسجلة في النظام</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-y border-slate-100">
                ...
                <th className="text-center px-4 py-3 text-xs font-bold text-slate-500">عدد الموظفين</th>
                <th className="text-center px-4 py-3 text-xs font-bold text-slate-500">مكتمل التدريب</th>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${dept.completionRate >= 80 ? 'bg-emerald-500' : dept.completionRate >= 60 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${dept.completionRate}%` }} />
                        </div>
                        <span className="text-xs font-bold text-slate-500 w-8 text-center">{dept.completionRate}%</span>
                      </div>
                    </td>
                  </tr>
                ))
              ) : (
                <tr><td colSpan={6} className="text-center py-8 text-slate-400 text-sm">لا توجد بيانات أقسام متاحة</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* ── ⭐ NEW: تفاصيل المشاركين - يدوي + AI ─────────────────────────── */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-sm">
            <Users size={16} className="text-indigo-500" />
            تفاصيل المشاركين — عرض يدوي + تحليل AI
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4 px-1">
          <div className="relative flex-1">
            ...
            <input type="text" value={participantSearch} onChange={e => setParticipantSearch(e.target.value)}
              placeholder="ابحث باسم الموظف أو البريد الإلكتروني أو القسم..."
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pr-9 pl-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-300" />
          </div>
          <select value={selectedCourseForParticipants} onChange={e => setSelectedCourseForParticipants(e.target.value)}
            className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-semibold text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-300">
            ...
            <tbody>
              {filteredParticipants.length > 0 ? (
                filteredParticipants.map((p, idx) => (
                  <tr key={`${p.employee.id}-${p.progress?.course_id || 'none'}-${idx}`}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-bold text-slate-800 text-xs">{p.employee.full_name}</p>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        <div className="w-12 h-1 bg-slate-100 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${(p.progress?.progress_percent || 0) >= 80 ? 'bg-emerald-500' : (p.progress?.progress_percent || 0) >= 40 ? 'bg-amber-500' : 'bg-red-500'}`}
                            style={{ width: `${p.progress?.progress_percent || 0}%` }} />
                        </div>
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={(e) => { e.stopPropagation(); setSelectedParticipant(p); }}
                        className="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-2 py-1 rounded-lg transition-colors">
                        <Eye size={11} /> تفاصيل
                      </button>
                    </td>
                  </tr>
                ...
                <tr><td colSpan={8} className="text-center py-8 text-slate-400 text-sm">لا توجد بيانات مشاركين متاحة</td></tr>
              )}
            </tbody>
          </table>
        </div>

        <div className="mt-4 p-3 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-indigo-100">
          <div className="flex items-center gap-2">
            <Brain size={16} className="text-purple-600" />
            <p className="text-xs text-slate-600"><span className="font-bold text-purple-700">✨ ميزة ذكية:</span> اضغط على أي مشارك لعرض التفاصيل الكاملة، ثم استخدم زر <span className="font-bold">"توليد تحليل بالذكاء الاصطناعي"</span> للحصول على تقرير تحليلي متكامل عن أدائه.</p>
          </div>
        </div>
      </Card>
        ...
        <div className="flex items-start gap-4 p-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              إجمالي {totalEnrolled} موظف مسجل في {courseStats.filter(c => c.enrolled > 0).length} دورة تدريبية.
              تم إتمام {totalCompleted} تسجيل بنسبة {overallCompletionRate}% وبمتوسط درجات {overallAvgScore}%.
              الدورات الإلزامية حققت نسبة إتمام {mandatoryRate}%. 
              هناك {totalInProgress} تسجيل قيد التنفيذ حالياً.
              {' '}— جميع الإحصائيات مبنية على بيانات حقيقية من النظام.
            </p>
          </div>
        </div>
      </Card>
      ...
      {selectedParticipant && (
        <ParticipantDetailModal
          ...
          onGenerateAI={() => handleGenerateAIAnalysis(selectedParticipant)}
        />
      )}
    </div>
  );
