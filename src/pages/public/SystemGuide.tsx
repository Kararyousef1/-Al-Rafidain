...
import { LayoutDashboard, Users, FileText, Heart, Bot, GraduationCap, ClipboardList, MessageSquare, BarChart3, Briefcase, ShieldCheck, UserCheck, Bell, ChevronLeft, X } from 'lucide-react';
...
const panels = [
  {
    ...
    features: [
      ...
      '🏆 سجلات الحضور - متابعة الحضور والانصراف',
  ...
  },
  {
    ...
    features: [
      ...
      '📢 التبليغات - نشر التبليغات والإشعارات',
      ...
      <div className="w-full max-w-4xl bg-white/5 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-white/10 shadow-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">
            مرحباً بك في نظام وادي الرافدين
          </h1>
          <p className="text-gray-400">
            لقد جهزنا لك هذه الخريطة لتعرف كيفية عمل النظام
          </p>
        </div>
          ...
          {panels.map((_, i) => (
            <div
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                i === currentIndex ? 'w-8 bg-indigo-500' : 'w-4 bg-white/20'
              }`}
            />
          ))}
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {current.features.map((feature, i) => (
              <div key={i} className="bg-white/5 rounded-xl px-4 py-3 border border-white/5 flex items-center gap-3">
                <span className="text-indigo-400 text-lg">{feature.split(' ')[0]}</span>
