    ...
    const fetchBadges = async () => {
      try {
        const [problemsRes, messagesRes] = await Promise.all([
          ...
          // ✅ إصلاح: العمود هو is_read وليس read
          supabase.from('notifications').select('id', { count: 'exact' }).eq('is_read', false).eq('user_id', user.id),
        ]);
        setDynamicBadges({
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          {sidebarOpen && (
            <div className="min-w-0">
              <p className="text-white font-bold text-sm leading-tight truncate">الرافدين</p>
